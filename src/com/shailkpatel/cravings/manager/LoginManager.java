...
public class LoginManager {
    ...
    public void login() {
        ...
        while (iv.isUniqueManagerPhoneNumber(phoneNumber)) {
            phoneAttempt++;
        ...
        while (attempts < MAX_ATTEMPTS) {
            ...
            if (!iv.isValidPassword(password)) {
                ...
                try {
                    ...
                    boolean isValid = dbConnector.validateManager(phoneNumber, encryptedPassword);
    ...
    private void processSuccessfulLogin(String phoneNumber) throws SQLException {
        ...
        Manager manager = dbConnector.getRestaurantManagerDBPhoneNumber(phoneNumber);
        ManagerMenu mm = new ManagerMenu();

        if (dbConnector.getManagerOnlineStatus(manager.getManagerId())) {
            System.out.println("There was an error in your last session!");
            System.out.println("Any unsaved work might have been deleted!");
        } else {
            dbConnector.updateManagerOnlineStatus(manager.getManagerId(), true);
        }

        if (!dbConnector.getManagerActiveStatus(manager.getManagerId())) {
            ...
            dbConnector.updateManagerActiveStatus(manager.getManagerId(), true);
        }
    ...
    private void forgotPhoneNumber() throws SQLException {
        ...
        while (iv.isUniqueManagerEmail(email)) {
            emailAttempt++;
        ...
        while (attempts < MAX_ATTEMPTS) {
            ...
            if (!iv.isValidPassword(password)) {
                ...
                try {
                    boolean found = dbConnector.validateManager(phoneNumber, encryptedPassword);
    ...
    private void forgotPassword(String phoneNumber) {
        ...
        try {
            DBConnectorManager dbConnector = new DBConnectorManager();
            boolean isValid = dbConnector.validateManagerFood(phoneNumber, favoriteFood);
